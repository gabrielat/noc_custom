...
class MetricScriptBase(BaseScriptMetaclass):
    ...
    def apply_snmp_rules(mcs, script):
        ...
        def sort_path_key(s):
            """
            M - Main, C - Custom, G - Enterasys.VH. P - profile
            \\|G|P
            -+-+-
            M|3|1
            C|2|0
            ...
            if s.startswith(PROFILES_PATH):
                return 3 if "Enterasys.VH. in s else 1
            else:
                return 2 if "Enterasys.VH. in s else 0
        ...
        if pp == "Enterasys.VH.:
            paths = [
                ...
                for p in config.get_customized_paths(
                    os.path.join("sa", "profiles", "Enterasys.VH., "snmp_metrics")
                )
            ]
        else:
            v, p = pp.split(".")
